...
app.post("/users", async function (req, res) {
	...
	if(!name || !handle || !password) {
		return res.status(400).json({msg: "required: name, handle and password"});
	}
	...
	try{
		const result = await xusers.insertOne({name, handle, profile, password: hash});
...
app.post("/login", async function(req, res) {
	...
	try {
		...
		if(user) {
			...
			if(result) {
				const token = jwt.sign(user, secret); // authentication
				return res.json({token, user});
			}
		}

		return res.json(403).json({msg: "Incorrect handle or password"});
	}catch {
		return res.sendFile(500);
	}
});

app.get("/posts", async function (req, res) { // middleware - auth
	try {
